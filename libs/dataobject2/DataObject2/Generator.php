<?php
/**
 * Generation tools for DB_DataObject2
 *
 * PHP version 5
 *
 ...
 * @version    CVS: $Id: Generator.php,v 1.117 2006/04/18 04:55:50 alan_k Exp $
 ...
 * Needed classes
 */
require_once realpath(dirname(__FILE__)).'/../DataObject2.php';
 ...
 * @package DB_DataObject2
 */
class DB_DataObject2_Generator extends DB_DataObject2
{
    /* =========================================================== */
     ...
     * @access private
     */
    private $table; // active tablename
    ...
    public function start()
    {
        ...
        $db_driver = empty($options['db_driver']) ? 'PDO' : $options['db_driver'];
    ...
    function _createTableList()
    {
        ...
        $__DB = DB_DataObject2::$CONNECTIONS[$this->_database_dsn_md5];
        ...
        if (empty($this->tables)) {
            ...
            throw new Exception('no tables',DB_DATAOBJECT_ERROR_NODATA);
        }

        //
        // TODO
        //
        // Views support removed cause mysql displays them anyway as tables
        //
    ...
    function generateDefinitions()
    {
        ...
        if (!is_array($options['schema_location'])) {
        	$options['schema_location'] = Array($options['schema_location']);
        }
        $base =  @$options['schema_location'][0];
    ...
    function _generateDefinitionsTable()
    {
        ...
        $keys_out_primary = '';
        $keys_out_secondary = '';
        if (DB_DataObject2::$CONFIG['debug'] > 2) {
            $this->debug("TABLE STRUCTURE FOR {$this->table}\n");
            $this->debug($defs);
        }
        ...
        if (@DB_DataObject2::$CONFIG['debug'] > 2) {
            $this->debug(array("dump for {$this->table}", $ret));
        }
    ...
    */
    ...
    function getClassNameFromTableName($table)
    {
        ...
        return  $class_prefix.preg_replace('/[^A-Z0-9]/i','_',ucfirst(trim($this->_database.'_'.$this->table)));
    }
    ...
    */
    ...
    {
        $options = &PEAR::getStaticProperty('DB_DataObject2','options');
        if (!is_array($options['class_location'])) {
        	$options['class_location'] = Array($options['schema_location']);
        }
        $base = $options['class_location'][0];
        ...
        if (strpos($base,'%s') !== false) {
            ...
            $outfilename = "{$base}/".preg_replace('/[^A-Z0-9]/i','_',ucfirst($this->_database.'_'.$this->table)).".php";
        }
        return $outfilename;
    ...
    */
    ...
    /**
     * class being extended (can be overridden by [DB_DataObject2_Generator] extends=xxxx
     *
     * @var    string
     * @access private
     */
    var $_extends = 'DB_DataObject2';

    /**
     * line to use for require('DataObject2.php');
     *
     * @var    string
     * @access private
     */
    ...
    function _generateClassTable($input = '')
    {
        ...
        // requires
        if (isset($this->_extendsFile)) {
        	if (!empty($this->_extendsFile)) {
        		$head .= "require_once '{$this->_extendsFile}';\n\n";
        	}
        } else {
        	$head .= "require_once DATAOBJECT2_PATH.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'DataObject2.php';\n\n";
        }
        ...
        $body .= "\n";
        $body .= "    /* Static get */\n";
        $body .= "    function staticGet(\$k,\$v=NULL) { return DB_DataObject2::staticGet('{$this->classname}',\$k,\$v); }\n";
    ...
    */
    function getProxyFull($database,$table) 
    {
        ...
        if ($extends_location = @$options['extends_location']) {
            $this->_extendsFile = $extends_location;
        }
    ...
    */
    function fillTableSchema($database,$table) 
    {
        ...
        DB_DataObject2::$INI[$database][$table] = $ret['table'];
        DB_DataObject2::$INI[$database][$table.'__keys'] = $ret['keys'];
        return false;
    ...
    */
    function _generateSequenceKeyFunction($def)
    {
        ...
        //print_r($def);
        // DB_DataObject2::debugLevel(5);
        global $_DB_DATAOBJECT;
        // print_r($def);
        
        
        // $dbtype     = DB_DataObject2::$CONNECTIONS[$this->_database_dsn_md5]->dsn['phptype'];
        $realkeys   = $def['keys'];
