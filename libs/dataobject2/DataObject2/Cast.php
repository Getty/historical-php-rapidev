...
* Common usages:
*   // blobs
*   $data = DB_DataObject2_Cast::blob($somefile);
*   $data = DB_DataObject2_Cast::string($somefile);
*   $dataObject->someblobfield = $data
*
*   // dates?
*   $d1 = new DB_DataObject2_Cast::date('12/12/2000');
*   $d2 = new DB_DataObject2_Cast::date(2000,12,30);
*   $d3 = new DB_DataObject2_Cast::date($d1->year, $d1->month+30, $d1->day+30);
*   
*   // time, datetime.. ?????????
*
*   // raw sql????
*    $data = DB_DataObject2_Cast::sql('cast("123123",datetime)');
*    $data = DB_DataObject2_Cast::sql('NULL');
*
...
class DB_DataObject2_Cast {
    ...
    */
    public function toStringFromBlob($to,$db) 
    {
        ...
        if (!($to & DB_DATAOBJECT_BLOB)) {
            return PEAR::raiseError('Invalid Cast from a DB_DataObject2_Cast::blob to something other than a blob!');
        }
        ...
        switch ($db->dsn["phptype"]) {
            ...
            default:
                return PEAR::raiseError("DB_DataObject2_Cast cant handle blobs for Database:{$db->dsn['phptype']} Yet");
        }
    ...
    */
    public function toStringFromString($to,$db) 
    {
        ...
        if (!($to & DB_DATAOBJECT_BLOB)) {
            return PEAR::raiseError('Invalid Cast from a DB_DataObject2_Cast::string to something other than a blob!'.
    ...
    */
    public function toStringFromDate($to,$db) 
    {
        ...
        if (($to !== false) && !($to & DB_DATAOBJECT_DATE)) {
            return PEAR::raiseError('Invalid Cast from a DB_DataObject2_Cast::string to something other than a date!'.
                ' (why not just use native features)');
        }
    ...
    public function toStringFromDateTime($to,$db) 
    {
        ...
        if (($to !== false) && 
            ...
            return PEAR::raiseError('Invalid Cast from a ' .
                ' DB_DataObject2_Cast::dateTime to something other than a datetime!' .
    ...
    */
    ...
    public function toStringFromTime($to,$db) 
    {
        ...
        if (($to !== false) && !($to & DB_DATAOBJECT_TIME)) {
            return PEAR::raiseError('Invalid Cast from a' . 
                ' DB_DataObject2_Cast::time to something other than a time!'.
                ' (try using native features)');
        }
